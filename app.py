...
def inject_user():
    best_selling ="yes"
    selling= list(mongo.db.Books.find({'book_best_selling': best_selling}).sort("_id", -1).limit(8))
...
def category_display(value_id):
    ...
    category_book_value= list(mongo.db.Books.find({'book_category_id': category_id['_id']}))
...
def book_update(book_id):
    if request.method == "POST":
        ...
        newvalues = { "$set": { "book_availabilty": request.form.get("book_availabilty"),"book_best_selling": request.form.get("book_best_selling"),"book_discount": request.form.get("book_discount"),"book_price": request.form.get("book_price"),"book_pages": request.form.get("book_pages") } }
...
def seereviews(book_id):
    ...
    review_book_value= list(mongo.db.book_reviews.find({'book_id': book_id['_id'],'review_status': "Approved" }))
